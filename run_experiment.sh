...
run_experiment()
{
    ...
    do
        ...
        do
            ...
            do
                curl -Ss "http://localhost:19999/api/v1/data?chart=cgroup_${NAME_CONTAINER}.${metric}&format=csv&after=-${TIME_COLLECTION}&group=max&points=30&options=nonzero" > $RESULTS_LOCAL_DIR/$NSERVICES/${NAME_CONTAINER}_${metric}_SRV${n}_CAM${n}_${EXPERIMENT_MACHINE}_${EXPERIMENT_SHIFT}_${SCENARIO_NAME}_${SEQUENCE}.csv
            done
        done
    done
